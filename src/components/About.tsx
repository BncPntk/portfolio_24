...
export function About() {
  ...
  return (
    <section class='max-w-[1152px] w-full px-2 justify-center flex flex-col mx-auto h-fit my-20'>
      ...
      <div
        id='about-me'
        class='grid md:grid-cols-2 items-center text-2xl text-blackSecondary justify-around gap-10 md:gap-16 lg:gap-28 px-6 md:px-0'
      >
        {/* left part */}
        <div class='text-start md:text-end font-bold md:pl-8 order-2 md:order-1'>
          <div class='pb-8 md:pb-12'>
            <h3 class='text-actionSecondary font-extrabold text-3xl pb-2 md:pb-3 text-start md:text-end'>
              DEVELOPMENT
            </h3>
            <ul>
              <For each={devSkills}>
                {(skill) => <li class='leading-normal md:leading-relaxed'>{skill}</li>}
              </For>
            </ul>
          </div>
          <div>
            <h3 class='text-actionSecondary font-extrabold text-3xl pb-2 md:pb-3 text-end md:text-end'>
              MULTIMEDIA TOOLS
            </h3>
            <ul>
              <For each={tools}>
                {(tool) => <li class='leading-normal md:leading-relaxed text-end'>{tool}</li>}
              </For>
            </ul>
          </div>
        </div>
        ...
        <div class='text-start text-xl md:text-lg lg:text-xl leading-normal md:leading-relaxed md:pr-8 order-1  md:order-2'>
          <h3 class='text-actionSecondary font-bold text-2xl pb-2'>WHO AM I?</h3>
          <p class='max-w-[600px]'>
